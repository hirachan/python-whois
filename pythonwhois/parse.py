...
registrant_regexes = [
	...
	"\[Registrant\]\s*(?P<organization>.*)\n", # .jp
	"g\. \[Organization\]               (?P<organization>.+)\n", # .co.jp registrations at jprs.jp
	"d\. \[Network Service Name\]\s*(?P<organization>.+)\n",  # .ne.jp registrations at jprs.jp
	...
	for obj in (registrant, tech_contact, billing_contact, admin_contact):
		if obj is not None:
			...
			if "phone1" in obj:
				...
				while True:
					try:
						phone_items.append(obj["phone%d" % i])
						del obj["phone%d" % i]
					except KeyError as e:
						break
					i += 1
			...
			if "email1" in obj:
				...
				while True:
					try:
						email_items.append(obj["email%d" % i])
						del obj["email%d" % i]
					except KeyError as e:
						break
					i += 1
